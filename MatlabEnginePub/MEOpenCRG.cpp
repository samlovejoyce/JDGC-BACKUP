...
#include <MatlabEnginePub/MEOpenCRG.h>

MatlabEngine::MEOpenCRG::MEOpenCRG()
	: pCrgData(nullptr)
	, pCrgDataCt(nullptr)
	, pCrgDataU(nullptr)
	, pCrgDataV(nullptr)
...
void MatlabEngine::MEOpenCRG::loadCrgData(const std::string crgFile)
{
	std::string command;
...
bool MatlabEngine::MEOpenCRG::saveCrgData(const std::string crgFile)
{
...
void MatlabEngine::MEOpenCRG::changeMatlabWorkPathToCRG(const std::string path /* = CRG_MATLAB_LIB_PATH*/)
{
	std::string command;
	command = "cd " + path + ";";
...
bool MatlabEngine::MEOpenCRG::createCrgDataFiled()
{
	if (pCrgData == nullptr)
	{
		int ndim = 1;
...
bool MatlabEngine::MEOpenCRG::setCrgDataComment(const std::string comment)
{
...
bool MatlabEngine::MEOpenCRG::setCrgDataZValues(double ** data)
{
	if (!pCrgData)
		return false;
...
bool MatlabEngine::MEOpenCRG::setCrgDataToMatlabEngine()
{
	if (!pCrgData)
	{
		return false;
...
void MatlabEngine::MEOpenCRG::setCrgDataStruct(CrgData * data)
{
	crgDataStruct = data;
}
